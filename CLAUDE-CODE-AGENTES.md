...
2. Peça: **Use o agente `ds-lead` para revisar o Button; ele deve delegar a inspeção ao `component-reviewer`. Não edite arquivos. Ao terminar, execute `npm run validate` na pasta raiz e apresente o status.** Troque `Button` por `Hyperlink`, `Dot`, `Checkbox` ou `Radio` para revisar os outros componentes do índice — os eixos de cada um estão em `agents/component-reviewer.md`.
